...
public class SysCompanyController extends BaseController {
    ...
    public Result update(@PathVariable("id") String id, @RequestBody SysCompanyEntity sysCompany){
    ...
    @RequiresRoles({"role:sys:admin"})
    public Result addAdminUser(@RequestParam("companyId") String companyId, @RequestBody SysUserEntity sysUser) {
        // 构建用户信息
        ...
        //给管理员角色分配权限
        List<SysPermissionEntity> sysPermission = sysPermissionDao.findAll();
    ...
    public Result assignAdminPermissions(@RequestParam("roleId") String roleId) {
        ...
        // 设置权限
        SysRoleEntity sysRoleEntity = sysRoleDao.findById(roleId).get();
        sysRoleEntity.setPermissions(new HashSet<>(sysPermission));
